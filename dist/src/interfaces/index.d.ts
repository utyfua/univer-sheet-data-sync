import { CellValue, CellValueType, ICellData, IDisposable, Nullable } from '@univerjs/core';
...
export type ICellSyncState<Row = unknown, NewRow extends object | false = NewRowBase | false> = {
    /**
     * @inner
     */
...
export type IColumn<Row = unknown, NewRow extends object | false = NewRowBase | false> = {
    innerHidden?: boolean;
    displayName?: string;
    rejectInvalidInput?: boolean;
    width?: number;
    extractCellData: (row: Row, column: IColumn<Row, NewRow>) => ICellSyncData;
    extractNewCellData?: (row: NewRow, column: IColumn<Row, NewRow>) => ICellSyncData;
    validateCellValue?: (value: Nullable<CellValue>, cell: ICellSyncState<Row, NewRow>) => boolean | string | undefined;
    onCellValueChange?: (value: Nullable<CellValue>, cell: ICellSyncState<Row, NewRow>) => void;
} & (NewRow extends object ? {
    key: keyof NewRow;
} : {
    key?: string;
});
export type ISyncOptions<Row = unknown, NewRow extends object | false = NewRowBase | false> = {
     ...
     */
    onNewRow?: (row: NewRow, cells: ICellSyncState<Row, NewRow>[]) => void | false | Row;
} : {
    freeRows?: undefined;
